...
class RandomizedSet {
  ...
  insert(value) {
    if (this.mapValues.has(value)) {
      return false;
    }
    this.mapValues.set(value, this.values.length);
  ...
  remove(value) {
    ...
    let valueIndex = this.mapValues.get(value);
...
randomizedSet.insert(1); // Inserts 1 to the set. Returns true as 1 was inserted successfully.
randomizedSet.insert(2); // Returns false as 2 does not exist in the set.
