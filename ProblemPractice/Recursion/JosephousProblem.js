...
function findLiveOne(arr, k, index, result) {
  if (arr.length == 1) {
    result.value = arr[0];
    return;
  }

  index = (index + k) % arr.length; // if index reached to end of array, using modulous can bring them back
  arr.splice(index, 1);

  findLiveOne(arr, k, index, result);
}
...
function main(n, k) {
  ...
  k = k - 1; // array index starting from 0

  let result = { value: 0 };
  findLiveOne(arr, k, 0, result);
  console.log('The live one is: ', result.value);
}
